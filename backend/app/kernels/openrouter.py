...
class OpenRouterAdapter(LLMKernelAdapter):
    def __init__(self):
    ...
    async def generate_chat_completion(self, messages: List[Dict[str, str]], temperature: float = 0.7) -> str:
        if not self.api_key:
            raise ValueError("OPENROUTER_API_KEY is not configured in settings (.env).")
        ...
        try:
            ...
            raise RuntimeError(f"Unexpected response structure from OpenRouter API: {data}")

    async def generate_embeddings(self, text: str) -> List[float]:
        # Fallback embeddings using Gemini if available, otherwise Ollama local
